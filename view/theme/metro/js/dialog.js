...
(function($) {
    $.Dialog = function(params) {
        ...
        $.each(params.buttons, function(name, obj) {
            ...
            buttonsHTML += '<button class="'+obj.cssclass+'">' + name + '</button>';
		...
		var rgba = 'rgba('+userthemecolour+', 1)';
		var str="How are you doing today?";
		var n=userthemecolour.split(","); 
		var hexc = rgba2hex(n[0],n[1],n[2]);
		$("#dialogBox .header").css({"background-color":ColorLuminance(hexc, -0.1)});
		$("#dialogBox .content").css({"background-color":ColorLuminance(hexc, -0.1)});
		$("#dialogBox .content input.inputtxtdialog").css({"border":"1px solid" + ColorLuminance(hexc, -0.2)});
		$("#dialogButtons").css({"background-color":ColorLuminance(hexc, -0.1)});
	$(".dialog .action button.add").css({"background-color":ColorLuminance(hexc, -0.30), "border":"1px solid" + ColorLuminance(hexc, 0.01)});
	$(".dialog .action button.close").css({"background-color":'transparent'});
        if(!params.overlay) {
